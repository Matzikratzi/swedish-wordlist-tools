...
class MissingBaselineAnchorTest(unittest.TestCase):
    def test_scan_order_is_top_then_left_and_baseline_comes_from_model(self):
        ...
        # ¤ starts on raster y=2 and therefore says baseline=4.  The x starts
        # lower down.  Unmatched ink above it must not stop the anchor search.
        ink = {
            (7, 0),
            (1, 2), (2, 2), (1, 3),
            (8, 4), (9, 4),
    ...
    def test_anchor_recovers_missing_baseline_and_keeps_anchor_in_partition(self):
        ...
        ink = {
            (1, 2), (2, 2), (1, 3),       # ¤ at baseline 4
            (5, 3), (6, 3), (6, 4),       # a at the same baseline
        }
        result = {
            "baseline": None,
            "ink": ink,

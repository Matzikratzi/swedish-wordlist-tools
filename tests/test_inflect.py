...
class InflectTests(unittest.TestCase):
    def test_generates_common_noun_forms(self) -> None:
        self.assertEqual(
            generate_forms("abakus", "+en +er"),
            ("abakus", "abakusen", "abakuser"),
        )
        self.assertEqual(
            generate_forms("abbé", "+n +er"),
            ("abbé", "abbén", "abbéer"),
        )

    def test_generates_adjective_and_verb_forms(self) -> None:
        self.assertEqual(
            generate_forms("abchazisk", "+t +a"),
            ("abchazisk", "abchaziskt", "abchaziska"),
        )
        self.assertEqual(
            generate_forms("abonnera", "+de +t"),
            ("abonnera", "abonnerade", "abonnerat"),
        )

    def test_plural_plus_does_not_duplicate_lemma(self) -> None:
        self.assertEqual(
            generate_forms("A-avdrag", "+et; pl. +"),
            ("A-avdrag", "A-avdraget"),
        )

    def test_rejects_missing_and_unsupported_patterns(self) -> None:
        self.assertIsNone(generate_forms("gå", None))
        self.assertIsNone(generate_forms("gå", "går gick gått"))

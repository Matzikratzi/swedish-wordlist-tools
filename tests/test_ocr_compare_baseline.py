...
class CompareBaselineTest(unittest.TestCase):
    ...
    def test_identical_stable_result_has_no_difference(self):
        row = {"page": 1, "column": 0, "row": 0, "baseline": 12, "text": "abc", "matches": [], "covered_pixels": 3, "source_pixels": 3, "fully_exact": True, "needs_work": False, "unreviewed_matches": 0, "crop_box": [0, 0, 3, 4]}
        with tempfile.TemporaryDirectory() as td:
            a, b = Path(td)/"a.jsonl", Path(td)/"b.jsonl"
            self._write(a, [row]); self._write(b, [{**row, "elapsed": 99.0}])
    ...
    def test_changed_ocr_result_is_reported(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            a, b = Path(td)/"a.jsonl", Path(td)/"b.jsonl"
            self._write(a, [row]); self._write(b, [{**row, "baseline": 13}])
            self.assertEqual(compare(a, b)[0]["kind"], "changed")

from __future__ import annotations

import unittest

from swedish_wordlist_tools.analyze_noun_article_saldo_alignment import analyze


class AnalyzeNounArticleSaldoAlignmentTests(unittest.TestCase):
    def test_unions_variant_paradigms_and_matching_saldo_analyses(self) -> None:
        noun_rows = [{
            "article_id": "5598",
            "record_id": "5598",
            "homonym_number": "1",
            "source_homonym_numbers": ["1", "0"],
            "lemma": "bankväsen",
            "variant_mode": "parallel_branches",
            "variant_lemmas": ["bankväsen", "bankväsende"],
            "variant_paradigms": [
                {"lemma": "bankväsen", "forms": [
                    {"written_form": "bankväsen"},
                    {"written_form": "bankväsendet"},
                ]},
                {"lemma": "bankväsende", "forms": [
                    {"written_form": "bankväsende"},
                    {"written_form": "bankväsendet"},
                ]},
            ],
        }]
        saldo = {
            "bankväsen": [{
                "id": "a", "upos": "NOUN", "lemmas": {"bankväsen"},
                "forms": {"bankväsen", "bankväsendet"},
            }],
            "bankväsende": [{
                "id": "b", "upos": "NOUN", "lemmas": {"bankväsende"},
                "forms": {"bankväsende", "bankväsendet"},
            }],
        }
        rows, summary = analyze(noun_rows, saldo)
        self.assertEqual(1, len(rows))
        self.assertEqual("exact", rows[0]["status"])
        self.assertEqual([], rows[0]["missing_variant_lemmas"])
        self.assertEqual(["exact", "exact"], [variant["status"] for variant in rows[0]["variants"]])
        self.assertEqual(1, summary["status_counts"]["exact"])
        self.assertEqual(2, summary["variant_status_counts"]["exact"])

    def test_reports_partial_variant_coverage_variant_by_variant(self) -> None:
        noun_rows = [{
            "article_id": "1",
            "record_id": "1",
            "homonym_number": "1",
            "lemma": "foo",
            "variant_mode": "shared_notation",
            "variant_lemmas": ["foo", "foe"],
            "variant_paradigms": [
                {"lemma": "foo", "forms": [{"written_form": "foo"}]},
                {"lemma": "foe", "forms": [{"written_form": "foe"}]},
            ],
        }]
        saldo = {
            "foo": [{"id": "a", "upos": "NOUN", "lemmas": {"foo"}, "forms": {"foo"}}]
        }
        rows, summary = analyze(noun_rows, saldo)
        self.assertEqual("saldo_subset", rows[0]["status"])
        self.assertEqual(["foe"], rows[0]["missing_variant_lemmas"])
        variants = {variant["lemma"]: variant for variant in rows[0]["variants"]}
        self.assertEqual("exact", variants["foo"]["status"])
        self.assertEqual("missing", variants["foe"]["status"])
        self.assertEqual(["foe"], variants["foe"]["only_in_saol"])
        self.assertEqual(1, summary["variant_coverage_counts"]["some_variants_missing"])

    def test_groups_legacy_duplicate_artifact_rows_by_article_id(self) -> None:
        common = {
            "record_id": "7", "lemma": "bar", "variant_mode": "shared_notation",
            "variant_lemmas": ["bar", "baar"],
            "variant_paradigms": [
                {"lemma": "bar", "forms": [{"written_form": "bar"}]},
                {"lemma": "baar", "forms": [{"written_form": "baar"}]},
            ],
        }
        noun_rows = [dict(common, homonym_number="1"), dict(common, homonym_number="0")]
        saldo = {
            "bar": [{"id": "a", "upos": "NOUN", "lemmas": {"bar"}, "forms": {"bar"}}],
            "baar": [{"id": "b", "upos": "NOUN", "lemmas": {"baar"}, "forms": {"baar"}}],
        }
        rows, summary = analyze(noun_rows, saldo)
        self.assertEqual(1, len(rows))
        self.assertEqual(1, summary["variant_articles"])
        self.assertEqual(2, summary["variant_paradigms"])


if __name__ == "__main__":
    unittest.main()

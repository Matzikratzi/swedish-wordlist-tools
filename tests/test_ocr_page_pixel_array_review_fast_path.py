...
class PagePixelArrayReviewFastPathTests(unittest.TestCase):
    def test_exact_row_never_runs_two_row_ownership(self):
        exact = {"fully_exact": True, "pixel_owner_revision": 0}
        with (
            ...
            result = review.load_review_state_pixel_array({"pixel_owner_revision": 0}, (1, 7), object())
    ...
    def test_defective_row_reanalyses_when_ownership_changes(self):
        defective = {"fully_exact": False, "pixel_owner_revision": 0}
        repaired = {"fully_exact": True, "pixel_owner_revision": 1}
        context = {"pixel_owner_revision": 1}
        with (
    ...
    def test_defective_row_reanalyses_if_parallel_owner_revision_changed(self):
        defective = {"fully_exact": False, "pixel_owner_revision": 0}
        refreshed = {"fully_exact": False, "pixel_owner_revision": 1}
        context = {"pixel_owner_revision": 1}
        with (
    ...
    def test_packet_refreshes_state_from_old_owner_revision(self):
        stale = {"fully_exact": False, "pixel_owner_revision": 0}
        fresh = {"fully_exact": True, "pixel_owner_revision": 1}
        fake_cache = Mock()
        fake_cache.get.return_value = fresh
        positions = [(1, 27), (1, 28)]
        original_states = [fresh, stale]
        old_context = review._current_pixel_context
        try:
            review._current_pixel_context = {"pixel_owner_revision": 1}
        ...
        self.assertEqual(result, [fresh, fresh])
        fake_cache.invalidate.assert_called_once_with((1, 28))
        fake_cache.get.assert_called_once_with((1, 28))

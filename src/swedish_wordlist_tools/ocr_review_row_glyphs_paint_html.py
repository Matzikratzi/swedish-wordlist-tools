...
<html lang="sv"><head><meta charset="utf-8"><title>SAOL glyphgranskning – hybridmask</title>
<style>
...
<p class="hint"><b>Hybridmarkering:</b> dra en rektangel som tidigare för att välja alla svarta källpixlar i området. Finjustera sedan med <b>Shift-klick</b> för att lägga till en enskild svart pixel och <b>Alt-klick</b> för att ta bort en vald pixel. Den streckade blå ramen visar bara minsta box runt den faktiska pixelmasken.</p>
<script>
const S={data};
...
function draw(){{
 ...
 if(dragStart&&dragNow){{const x=Math.min(dragStart.x,dragNow.x)*scale,y=topPad+Math.min(dragStart.y,dragNow.y)*scale,w=(Math.abs(dragStart.x-dragNow.x)+1)*scale,h=(Math.abs(dragStart.y-dragNow.y)+1)*scale;ctx.save();ctx.strokeStyle='#1769d2';ctx.lineWidth=2;ctx.strokeRect(x,y,w,h);ctx.restore();}}
}}
function sync(){{
...
function addRectangle(a,b){{
 const x0=Math.min(a.x,b.x),x1=Math.max(a.x,b.x),y0=Math.min(a.y,b.y),y1=Math.max(a.y,b.y);
 for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){{const key=x+','+y;if(sourceInk.has(key))chosenPixels.add(key);}}
}}
function touchPixel(p, mode){{
 const key=p.x+','+p.y;
 if(!sourceInk.has(key))return;
 if(mode==='add')chosenPixels.add(key);
 if(mode==='remove')chosenPixels.delete(key);
}}
...
canvas.addEventListener('mousedown',e=>{{
 ...
 if(e.shiftKey){{touchPixel(p,'add');sync();e.preventDefault();return;}}
 if(e.altKey){{touchPixel(p,'remove');sync();e.preventDefault();return;}}
 dragStart=p;dragNow=p;e.preventDefault();draw();
}});
canvas.addEventListener('mousemove',e=>{{if(!dragStart)return;dragNow=canvasPixel(e);draw();}});
window.addEventListener('mouseup',e=>{{
 if(!dragStart)return;
 dragNow=canvasPixel(e);addRectangle(dragStart,dragNow);dragStart=null;dragNow=null;sync();
}});
canvas.addEventListener('click',e=>{{
 if(e.shiftKey||e.altKey)return;
 const p=canvasPixel(e);const hits=S.items.filter(it=>p.x>=it.bbox.left&&p.x<it.bbox.right&&p.y>=it.bbox.top&&p.y<it.bbox.bottom);if(hits.length&&chosenPixels.size===0)toggle(hits[hits.length-1].id);
}});

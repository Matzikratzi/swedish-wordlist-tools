...
from .ocr_profile_gap_filter import filter_horizontal_blank_bands
...
from .ocr_whole_column_profile import build_profile_fragment_index, profile_guided_exact_hits, walk_profile_row_starts, whole_column_left_profile


def main() -> int:
    ap = argparse.ArgumentParser(description="Whole-column left-profile shadow with clear horizontal glyph-gap verification.")
    ...
    fragment_index = build_profile_fragment_index(models, min_rows=args.min_profile_rows, max_rows=args.max_profile_rows, min_ink_rows=args.min_profile_ink_rows)
    print(f"profile-gap-models: models={len(models)} build={perf_counter()-started:.4f}s", flush=True)
    ...
    profile = whole_column_left_profile(black, min_y=top, max_y=bottom - 1, min_x=0, max_x=args.profile_max_x)
    if args.dump_profile:
        for offset, x in enumerate(profile):
            print(f"profile-y: y={top+offset} left_x={'none' if x is None else x}")

    started = perf_counter()
    raw_hits = profile_guided_exact_hits(
        black,
        profile,
        profile_min_y=top,
    ...
    hits = filter_horizontal_blank_bands(black, raw_hits)
    print(
        f"profile-gap-hits: raw={len(raw_hits)} accepted={len(hits)} rejected={len(raw_hits)-len(hits)} search={perf_counter()-started:.4f}s",
        flush=True,
    )

    rows = walk_profile_row_starts(hits, start_y=top, end_y=bottom - 1, max_row_distance=args.max_row_distance, min_baseline_delta=args.min_baseline_delta)
    for row in rows:
        hit = row.hit
        print(
            f"profile-gap-row: new={row.index} search={row.search_from_y}..{row.search_to_y} profile={hit.profile_start_y}..{hit.profile_end_y} "
            f"top={hit.top_y} baseline={hit.baseline} bottom={hit.bottom_y} start={hit.model.label!r}/{hit.model.style}@x{hit.x} "
            f"profile_rows={hit.profile_rows} ink_rows={hit.profile_ink_rows} glyph_pixels={len(hit.model.pixels)} next={row.next_search_y}",
            flush=True,
        )
    print(f"profile-gap-summary: rows={len(rows)}", flush=True)
    return 0

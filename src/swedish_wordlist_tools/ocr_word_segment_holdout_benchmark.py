...
from .ocr_glyph_consensus_match import classify_consensus
...
def _materialize_refs(
    ...
    for row in words:
        ...
        for glyph in row.get("glyphs", []):
            ...
            # Avoid copying bytes; Pillow save also normalizes the local file reference.
...
def main() -> int:
    ...
    ap.add_argument("--limit", type=int, default=0, help="Maximum held-out words; 0 means all")
    ...
    results = []
    skipped = Counter()
    word_correct = char_correct = char_total = 0
    raw_char_correct = 0
    ...
    with tempfile.TemporaryDirectory(prefix="saol-word-holdout-") as tmp:
        ...
        for row in words:
            ...
            refs, independent = _materialize_refs(
                args.library,
                list(manifest.get("words", [])),
                source_id,
                subnr,
                tmp_root / f"holdout-{source_id:05d}",
            )
            ref_classes = {p.name.split("-", 1)[0] for p in refs}
            # filenames use uXXXX for punctuation; semantic presence is easier from independent map.
            ...
            results.append({
                ...
                "char_correct": n,
                "char_total": len(expected),
                "independent_sources_by_class": independent,
                "segments": segrows,
            })
    ...
    payload = {
        ...
        "notes": {
            ...
            "segmentation": "training glyphs were mined from whole known words; test word is segmented by the same _segment_word routine",
            "classifier": "per-class aligned consensus with geometry prior",
        },
    }

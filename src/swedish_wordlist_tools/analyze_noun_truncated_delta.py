...
EXPECTED_RECORD_DELTA = 3
EXPECTED_FORM_DELTA = 20
...
def build_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    truncated_records = before_records = after_records = before_forms = after_forms = 0
    for record in records:
        if str(record.get("upos") or "").upper() != "NOUN" or not is_truncated_inflection_source(record):
            continue
        truncated_records += 1
    ...
    record_delta = after_records - before_records
    form_delta = after_forms - before_forms
    return {
        "truncated_records": truncated_records,
        "before_generated_records": before_records,
        "after_generated_records": after_records,
        "generated_record_delta": record_delta,
        "before_form_rows": before_forms,
        "after_form_rows": after_forms,
        "form_row_delta": form_delta,
        "matches_observed_global_delta": record_delta == EXPECTED_RECORD_DELTA and form_delta == EXPECTED_FORM_DELTA,
...
def render_text(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"Matchar observerad global delta (+3/+20): {'JA' if summary['matches_observed_global_delta'] else 'NEJ'}",
    ...
    for index, row in enumerate(summary["rows"], start=1):
        ...
        lines.extend(["", f"{index}. {row['lemma']}{homonym} | id={row['record_id']}", f"   text={row['text']!r}", f"   före: {'genererad' if row['before_generated'] else 'ej genererad'} ({row['before_form_count']} formrader)", f"   efter: {'genererad' if row['after_generated'] else 'ej genererad'} ({row['after_form_count']} formrader)"])
        if row["recovered_key_forms"]:
            lines.append("   säkert återvunna key forms:")
            for form in row["recovered_key_forms"]:
                lines.append(f"     {form['slot']}: {form['written_form']} <- {form['source_token']!r}")
        if row["added_forms"]:
            lines.append("   tillkomna formrader:")
            for form in row["added_forms"]:
                lines.append(f"     {form['written_form']} | {form['msd']} | {form['kind']}")
...
def main() -> None:
    ...
    print(f"Matchar observerad global delta (+3/+20): {'JA' if summary['matches_observed_global_delta'] else 'NEJ'}")

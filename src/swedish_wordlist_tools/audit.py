...
def render_html(
    ...
    for section_index, pattern in enumerate(pattern_order(samples)):
        ...
        for row_index, row in enumerate(rows):
            ...
            flags = " ".join(f'<span class="flag">{html.escape(flag)}</span>' for flag in row.flags) or '<span class="muted">inga</span>'
        ...
        button = f'<button type="button" class="more" data-target="pattern-{section_index}" data-batch="{batch_size}">Nästa {batch_size} exempel</button>' if len(rows) > batch_size else ""
        sections.append(
            ...
            + "".join(body_rows) + f'</tbody></table><p>{button}</p></section>'
        )
    flag_summary = "".join(f'<li><code>{html.escape(flag)}</code>: {count}</li>' for flag, count in sorted(flag_counts.items(), key=lambda item: (-item[1], item[0]))) or "<li>Inga flaggor</li>"
...
radios.forEach(r => r.addEventListener('change', () => {{ reviews[r.name] = r.value; localStorage.setItem(storageKey, JSON.stringify(reviews)); refresh(); }}));
...
def build_audit(input_path: Path, output_path: Path, examples_per_pattern: int = DEFAULT_EXAMPLES, batch_size: int = DEFAULT_BATCH_SIZE, seed: int = DEFAULT_SEED) -> dict[str, Any]:
    samples, pattern_counts, flag_counts = sample_rows(read_jsonl(input_path), examples_per_pattern, seed)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(render_html(samples, pattern_counts, flag_counts, examples_per_pattern, batch_size, seed), encoding="utf-8")
    return {"output": str(output_path), "supported_records": sum(pattern_counts.values()), "sampled_records": sum(map(len, samples.values())), "patterns": len(pattern_counts), "pattern_counts": pattern_counts, "flag_counts": flag_counts, "examples_per_pattern": examples_per_pattern, "batch_size": batch_size, "seed": seed}

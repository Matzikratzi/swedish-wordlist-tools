...
function componentsOf(inkSet){{
 const unseen=new Set(inkSet),components=[],componentByPixel=new Map();
 while(unseen.size){{
  const seed=unseen.values().next().value;unseen.delete(seed);const comp=new Set([seed]),stack=[seed];
  while(stack.length){{const k=stack.pop(),[x,y]=k.split(',').map(Number);for(const nk of [pkey(x-1,y),pkey(x+1,y),pkey(x,y-1),pkey(x,y+1)])if(unseen.has(nk)){{unseen.delete(nk);comp.add(nk);stack.push(nk);}}}}
  const idx=components.length;components.push(comp);for(const k of comp)componentByPixel.set(k,idx);
 }}
...
for(const c of DATA.candidates){{
 ...
 let baseline=Number.isInteger(row.baseline)?row.baseline:Math.max(0,row.height-2),dragBaseline=false,dragRect=false,rect=null,rectMode='replace',pixelMode=null;
 ...
 const {{components,componentByPixel}}=componentsOf(inkSet);
 const selected=new Set((row.candidate_pixels||[]).map(([x,y])=>pkey(x,y)));
 function applyComponentAt(k,mode){{const idx=componentByPixel.get(k);if(idx===undefined)return;for(const pk of components[idx]){{if(mode==='remove')selected.delete(pk);else selected.add(pk);}}}}
 function toggleComponentAt(k){{const idx=componentByPixel.get(k);if(idx===undefined)return;const comp=components[idx],allSelected=[...comp].every(pk=>selected.has(pk));for(const pk of comp){{if(allSelected)selected.delete(pk);else selected.add(pk);}}}}
 ...
 function editPixel(p,mode){{const k=pkey(p.x,p.y);if(!inkSet.has(k))return;if(mode==='add')selected.add(k);else selected.delete(k);}}
 function applyRect(){{if(!rect)return;const x0=Math.min(rect.x0,rect.x1),x1=Math.max(rect.x0,rect.x1),y0=Math.min(rect.y0,rect.y1),y1=Math.max(rect.y0,rect.y1);if(rectMode==='replace')selected.clear();const touched=new Set();for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){{const idx=componentByPixel.get(pkey(x,y));if(idx!==undefined)touched.add(idx);}}for(const idx of touched)for(const pk of components[idx]){{if(rectMode==='remove')selected.delete(pk);else selected.add(pk);}}}}
 canvas.onmousedown=e=>{{const p=pt(e),k=pkey(p.x,p.y);if(e.ctrlKey&&(e.shiftKey||e.altKey)){{pixelMode=e.altKey?'remove':'add';editPixel(p,pixelMode);draw();return;}}if(Math.abs(p.cy-baseY())<=Math.max(6,SCALE/2)){{dragBaseline=true;return;}}if(!e.shiftKey&&!e.altKey&&inkSet.has(k)){{toggleComponentAt(k);draw();return;}}dragRect=true;rectMode=e.altKey?'remove':(e.shiftKey?'add':'replace');rect={{x0:p.x,y0:p.y,x1:p.x,y1:p.y}};draw();}};
 canvas.onmousemove=e=>{{const p=pt(e);if(pixelMode){{editPixel(p,pixelMode);draw();return;}}if(dragBaseline){{baseline=Math.max(0,Math.min(row.height-1,Math.round(p.cy/SCALE-M-1)));draw();return;}}if(dragRect){{rect.x1=p.x;rect.y1=p.y;draw();}}}};
 canvas.onmouseup=()=>{{if(dragRect)applyRect();dragRect=false;dragBaseline=false;pixelMode=null;rect=null;draw();}};canvas.onmouseleave=canvas.onmouseup;

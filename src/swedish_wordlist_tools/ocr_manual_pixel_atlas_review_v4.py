...
def _guess_baseline(path: Path, threshold: int = 210) -> tuple[int, int, float]:
    """Return (raw_guess, adjusted_guess, confidence).

    The previous estimator was almost always one raster row too high.  Keep its
    modal-bottom logic, but use raw+1 as the editor default.  The raw value is
    retained in metadata so this correction remains explicit and auditable.
    """
    ...
    for x in range(im.width):
        ...
        if ys:
            lows.append(max(ys))
    if not lows:
        raw = max(0, im.height - 2)
        return raw, min(im.height - 1, raw + 1), 0.0
    counts = Counter(lows)
    scores = {
    ...
    adjusted = min(im.height - 1, raw + 1)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Hybrid manual atlas v4: box ink, edit pixels, corrected baseline, facsimile links.")
    ap.add_argument("library", type=Path)
    ap.add_argument("--out", type=Path, required=True)
    ap.add_argument("--atlas", type=Path)
    ap.add_argument("--jsonl", type=Path, help="SAOL JSONL; supplies per-subnr facsimile source links")
    ...
    for w in words:
        ...
        raw_baseline, baseline, confidence = _guess_baseline(path, args.ink_threshold)
        expected = str(w.get("expected_word") or "")
        headword = str(w.get("headword") or "")
        style = str(w.get("style") or "")
        subnr = str(w.get("subnr") or "")
        source = sources.get(subnr)
        source_html = (
            f'<a class="facsimile" href="{html.escape(source, quote=True)}" target="_blank" rel="noopener">Öppna faksimil ↗</a>'
            if source else '<span class="nosource">faksimillänk saknas</span>'
        )
        cards.append(f'''<article class="card" data-source-id="{html.escape(str(w.get('source_id') or ''), quote=True)}" data-style="{html.escape(style, quote=True)}" data-expected="{html.escape(expected, quote=True)}" data-headword="{html.escape(headword, quote=True)}" data-word-file="{html.escape(rel, quote=True)}" data-page="{html.escape(str(w.get('page') or ''), quote=True)}" data-subnr="{html.escape(subnr, quote=True)}" data-w="{width}" data-h="{height}" data-baseline="{baseline}" data-baseline-raw="{raw_baseline}" data-baseline-confidence="{confidence:.4f}">
<header><strong>{html.escape(expected)}</strong><span class="badge">{html.escape(style)}</span><span>uppslagsord: <b>{html.escape(headword) if headword else '(saknas i denna batch)'}</b></span><span>sida {html.escape(str(w.get('page') or ''))} · subnr {html.escape(subnr)}</span>{source_html}</header>
<div class="controls"><label>Etikett <input class="label" size="8" placeholder="a / i / st"></label><button class="boxmode active" type="button">Boxläge</button><button class="pixelmode" type="button">Pixelläge</button><label>Bläck &lt; <input class="threshold" type="number" min="0" max="255" value="{args.ink_threshold}" size="4"></label><label>Baslinje y=<input class="baseline" type="number" min="0" max="{height-1}" value="{baseline}"></label><button class="up" type="button">−1</button><button class="down" type="button">+1</button><span class="baseinfo">auto rå {raw_baseline} → standard {baseline} · conf {confidence:.2f}</span><button class="erase" type="button">Sudd: av</button><button class="clear" type="button">Rensa</button><span class="count"></span></div>

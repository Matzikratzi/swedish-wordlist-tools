...
def _analyse_one(path: Path, models) -> dict[str, Any]:
    ...
    # For incomplete words, infer one word-wide baseline from exact matches, then
    # regenerate candidates strictly on that baseline.  Nothing fuzzy is ever
    # shown: a glyph is highlighted only when a stored model matches pixel for
    # pixel at the chosen baseline.
...
<div class='top'>
 ...
 <div><small>Endast 100 % exakta facitmodeller markeras. Om en glyph inte matchar perfekt lämnas dess pixlar svarta och omarkerade. En gemensam stödlinje används för hela ordet.</small></div>
</div>
<div id='cards'></div>
<script>
const DATA={payload};
...
function renderCard(row){{
 ...
 const info=document.createElement('div');info.className='meta';info.textContent='Baseline: '+(row.baseline??'okänd')+' · perfekta glyphar: '+row.exact.map(m=>m.label+'@'+m.x).join('  ');d.appendChild(info);
 ...
 function draw(){{
   ...
   // Source raster. Exact matches get a visible fill; unmatched ink stays black.
   for(const [x,y] of row.ink){{
     ctx.fillStyle=exactPixels.has(x+','+y)?'#8f83d8':'#111';
     ctx.fillRect((x+M)*SCALE,(y+M)*SCALE,SCALE,SCALE);
   }}
   if(Number.isInteger(row.baseline)){{
     const yy=(row.baseline+1+M)*SCALE;ctx.strokeStyle='#d33';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,yy);ctx.lineTo(c.width,yy);ctx.stroke();
   }}
 ...
 c.onmousedown=e=>{{const p=pt(e);drag=sel={{x0:p.x,y0:p.y,x1:p.x,y1:p.y}};draw();}};
 c.onmousemove=e=>{{if(!drag)return;const p=pt(e);drag.x1=p.x;drag.y1=p.y;draw();}};
 ...
 ctrl.querySelector('.add').onclick=()=>{{
   ...
   if(!Number.isInteger(row.baseline)){{ctrl.querySelector('.msg').textContent='Ingen säker baseline för ordet.';return;}}
   ...
   const rel=pts.map(([x,y])=>[x-minx,y-row.baseline]).sort((a,b)=>a[0]-b[0]||a[1]-b[1]);
   ...
   additions.push(g);ctrl.querySelector('.msg').textContent='Tillagd: '+label+' '+style.value+' ('+rel.length+' pixlar).';d.classList.add('added');stats();
 }};
 draw();return d;
}}

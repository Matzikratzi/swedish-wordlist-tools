...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Correct auto-proposed pixel candidates with a DOM pixel grid."
    )
...
<div class="proposal-list"></div><div class="legend"></div>
    ...
    doc = f'''<!doctype html><meta charset="utf-8"><title>SAOL candidate editor v3</title>
<style>
...
.stack{{position:relative;display:block;overflow:hidden;border:1px solid #999;background:white}}
...
.baseline-line{{position:absolute;left:0;right:0;height:2px;background:#e00000;z-index:4;pointer-events:none}}
...
.accepted{{background:#e8f7eb}}.rejected{{background:#fff3cd}}.selected{{outline:2px solid #1479ff}}.legend,.imginfo{{font-size:12px;color:#555;margin-top:6px}}
</style>
<h1>SAOL autoannotering → manuell korrigering v3</h1>
<p>Den här versionen använder inget canvas. Bilden är ett vanligt IMG-lager, varje rasterpixel är en separat klickbar HTML-cell, och den röda baslinjen är ett eget element. Visningsram: {margin} px.</p>
...
document.querySelectorAll('.card').forEach(card=>{{
 ...
 const grid=card.querySelector('.grid'),line=card.querySelector('.baseline-line');
 ...
 for(let fy=0;fy<FH;fy++)for(let fx=0;fx<FW;fx++){{
   ...
   cell.onclick=()=>{{if(state.selected===null)return;if(x<0||y<0||x>=W||y>=H)return;const p=proposals[state.selected],k=x+','+y;if(p.pixels.has(k))p.pixels.delete(k);else p.pixels.add(k);render();}};
...
document.querySelector('#export').onclick=()=>{{
 ...
 const out={{format:'saol-manual-pixel-atlas-corrected-v3',coordinate_system:'original word crop origin top-left; y_rel = y - baseline_y',words}};
 const blob=new Blob([JSON.stringify(out,null,2)+'\\n'],{{type:'application/json'}}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='saol14-manual-pixel-atlas-corrected-v3.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)
}};
</script>'''

...
def _load_template_sources(library: Path) -> dict[str, dict[str, object]]:
    manifest = library / "manifest-pages.json"
...
def main() -> int:
    ...
    template_sources = _load_template_sources(args.library)
            ...
            for path in groups[label]:
                ...
                if entry:
                    ...
                    if isinstance(source, str) and source:
                        source_html = f'<a class="source" href="{html.escape(source, quote=True)}" target="_blank" rel="noopener">hela källsidan {html.escape(str(page))} ↗</a><div class="sourceword">{html.escape(str(word))}</div>'
...
const modal=document.getElementById('modal'), img=document.getElementById('sourceimg'), canvas=document.getElementById('overlay'), ctx=canvas.getContext('2d'), coords=document.getElementById('coords'), title=document.getElementById('modaltitle');
...
'''
    ...
    print(args.out)
    print(f"templates={total} styles={','.join(styles)} source_links={'yes' if sources else 'no'} source_contexts={source_contexts}")
    return 0

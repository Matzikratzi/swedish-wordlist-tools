...
def _svg(points: list[tuple[int, int]], scale: int = 10) -> str:
    ...
    left, right = min(xs), max(xs); top, bottom = min(ys), max(ys); pad = 2
    width = right - left + 1 + 2 * pad; height = bottom - top + 1 + 2 * pad
    rects = []
    for x, y in points:
        rx = (x - left + pad) * scale; ry = (y - top + pad) * scale
        rects.append(f'<rect x="{rx}" y="{ry}" width="{scale}" height="{scale}"/>')
    baseline_y = (0 - top + pad + 1) * scale; baseline = ""
    if top - pad <= 0 <= bottom + pad:
...
def render(facit: Path, labels: list[str]) -> str:
    data = json.loads(facit.read_text(encoding="utf-8")); models = data.get("glyphs") or []; wanted = set(labels)
    grouped: dict[str, dict[str, list[tuple[int, dict]]]] = defaultdict(lambda: defaultdict(list))
    for index, model in enumerate(models):
        label = str(model.get("label") or "")
        if label in wanted: grouped[label][_role(model)].append((index, model))

    sections = []
    for label in labels:
        roles = grouped.get(label, {}); role_html = []
        for role in sorted(roles):
            ...
            for index, model in roles[role]:
                ...
                cards.append(
                    ...
                    + f'<button onclick="savePng(this,\'{stem}.png\')">PNG</button>'
                    + f'<button data-json="{payload}" onclick="saveJson(this,\'{stem}.json\')">JSON</button>'
                    + f'<button data-json="{payload}" onclick="copyJson(this)">Kopiera JSON</button>'
                    + '</div></div>'
                )
            role_html.append(f'<h3>{html.escape(role)}</h3><div class="cards">{"".join(cards)}</div>')
        if not role_html: role_html.append('<p>Inga modeller i facit.</p>')
        sections.append(f'<section><h2>{html.escape(label)}</h2>{"".join(role_html)}</section>')

    return f'''<!doctype html><html lang="sv"><head><meta charset="utf-8"><title>SAOL glyph variants</title>
<style>
...
function saveSvg(btn,name){{const s=cardSvg(btn).cloneNode(true);s.setAttribute('xmlns','http://www.w3.org/2000/svg');download(name,new Blob([new XMLSerializer().serializeToString(s)],{{type:'image/svg+xml'}}))}}
function savePng(btn,name){{const s=cardSvg(btn).cloneNode(true);s.setAttribute('xmlns','http://www.w3.org/2000/svg');const text=new XMLSerializer().serializeToString(s);const img=new Image();const u=URL.createObjectURL(new Blob([text],{{type:'image/svg+xml'}}));img.onload=()=>{{const c=document.createElement('canvas');c.width=img.width;c.height=img.height;const x=c.getContext('2d');x.fillStyle='white';x.fillRect(0,0,c.width,c.height);x.drawImage(img,0,0);URL.revokeObjectURL(u);c.toBlob(b=>download(name,b),'image/png')}};img.src=u}}
function jsonText(btn){{return btn.dataset.json}}
function saveJson(btn,name){{download(name,new Blob([jsonText(btn)+'\\n'],{{type:'application/json'}}))}}
async function copyJson(btn){{await navigator.clipboard.writeText(jsonText(btn));const old=btn.textContent;btn.textContent='Kopierat';setTimeout(()=>btn.textContent=old,900)}}
</script></body></html>'''


def main() -> None:
    parser = argparse.ArgumentParser(description="Visa alla facitvarianter för valda glypher")
    parser.add_argument("--facit", required=True, type=Path); parser.add_argument("--glyph", action="append", required=True, dest="glyphs")
    parser.add_argument("--output", type=Path, help="Skriv HTML hit i stället för temporär fil"); parser.add_argument("--no-open", action="store_true", help="Öppna inte webbläsaren")
    args = parser.parse_args(); page = render(args.facit, args.glyphs)
    if args.output: target = args.output; target.write_text(page, encoding="utf-8")
    else:
        handle = tempfile.NamedTemporaryFile(prefix="saol-glyph-variants-", suffix=".html", delete=False); target = Path(handle.name); handle.close(); target.write_text(page, encoding="utf-8")
    print(target)

...
def _row_record(page: int, position: tuple[int, int], state: dict) -> dict:
    ...
    return {
        "page": int(page), "column": int(position[0]), "row": int(position[1]),
        "crop_box": [int(v) for v in state.get("crop_box") or ()],
        "baseline": None if state.get("baseline") is None else int(state["baseline"]),
        "covered_pixels": work.covered_pixels, "source_pixels": work.source_pixels,
        "fully_exact": work.fully_exact, "needs_work": work.needs_work,
        "unreviewed_matches": work.unreviewed_matches,
        "text": str(state.get("text") or ""),
        "matches": [_match_record(m) for m in matches],
    }
...
def main() -> int:
    ...
    ap.add_argument("--expect-facit-models", type=int)
    ...
    def capture(page, position, state):
        ...
        record = _row_record(page, position, state)
    ...
    scanner.classify_row_state = capture
    old_argv = sys.argv
    ...
    summary = {
        "format": "saol14-ocr-baseline-summary-v1",
        "pages": args.end_page - args.start_page + 1,
        "rows": len(all_rows),
        "exact": sum(bool(r["fully_exact"]) for r in all_rows),
        "needs_work": len(problems),
        "facit_models": model_count,
    }
    ...
    manifest = {
        ...
        "parameters": {"start_page": args.start_page, "end_page": args.end_page, "threshold": args.threshold},
        "source": {"jsonl": str(args.jsonl), "sha256": _sha256_file(args.jsonl)},
        "facit": {"aggregate": str(args.facit), "aggregate_sha256": _sha256_file(args.facit), "split_store": str(split), "split_store_sha256": _sha256_tree(split), "models": model_count, "verified_equal": True},
        "outputs": {name: {"path": path.name, "sha256": _sha256_file(path)} for name, path in (("rows", rows_path), ("problems", problems_path), ("summary", summary_path))},
    }

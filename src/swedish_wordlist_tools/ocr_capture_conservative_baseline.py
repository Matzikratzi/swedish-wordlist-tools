...
def _facit_model_count(payload):return len(payload.get("glyphs") or [])
def _match_record(m):
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument("jsonl",type=Path);ap.add_argument("--facit",type=Path,required=True);ap.add_argument("--start-page",type=int,required=True);ap.add_argument("--end-page",type=int,required=True);ap.add_argument("--threshold",type=int,default=210);ap.add_argument("--expect-facit-models",type=int);ap.add_argument("--output-dir",type=Path,required=True);a=ap.parse_args();split=a.facit.parent/"facit-v2";verify_facit(a.facit,split);payload=load_split_facit(split);n=_facit_model_count(payload)
 ...
 def capture(page,pos,state):
  ...
  if work.needs_work:problems.append(rec)
  return work

...
def main() -> int:
    ...
    filtered["review_hidden_small_residual_count"] = hidden
    filtered["review_min_orphan"] = args.min_orphan

    tmp = args.decode.with_name(args.decode.stem + ".review-v4-filtered.json")
    ...
    for row in kept:
        ...
        bits = []
        if missing:
            bits.append("SAKNAR FACIT: " + " ".join(map(str, missing)))
        if orphan:

            ...
            for item in data.get("templates", []):
                ...
                if isinstance(output, str) and isinstance(bbox, (list, tuple)) and len(bbox) == 4:
                    ...
                    template_sources[output] = {
                        ...
                        "bbox": list(bbox),
                        "page_bbox": page_bbox,
                        "word_bbox": item.get("word_bbox"),
                        "page_word_bbox": ([int(item["word_bbox"][0]) + column_left, int(item["word_bbox"][1]), int(item["word_bbox"][2]), int(item["word_bbox"][3])] if isinstance(item.get("word_bbox"), (list, tuple)) and len(item["word_bbox"]) == 4 else None),

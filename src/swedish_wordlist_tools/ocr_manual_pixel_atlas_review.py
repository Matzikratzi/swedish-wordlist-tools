...
.legend{{font-size:12px;margin-top:6px;display:flex;gap:8px;flex-wrap:wrap}}.swatch{{padding:2px 6px;border-radius:4px;color:white;text-shadow:0 1px 2px #000}}.count{{color:#555;font-size:12px}}#q{{width:24rem;max-width:80vw;padding:.4rem}}.hidden{{display:none}}
</style>
<h1>SAOL manuell pixelatlas</h1>
<p>Varje klick färgar exakt <b>en originalpixel</b>. Dra med musknappen nere för flera pixlar. Välj etikett först. Samma etikett kan märka flera separata delar, t.ex. stammen och pricken i <b>i</b>. Baslinjen exporteras tillsammans med pixelkoordinaterna.</p>
...
function setup(card){{
 ...
 function render(){{
  ...
  ctx.lineWidth=1;ctx.strokeStyle='rgba(100,100,100,.20)';
  for(let x=0;x<=W;x++){{ctx.beginPath();ctx.moveTo(x*SCALE+.5,0);ctx.lineTo(x*SCALE+.5,H*SCALE);ctx.stroke();}}
  for(let y=0;y<=H;y++){{ctx.beginPath();ctx.moveTo(0,y*SCALE+.5);ctx.lineTo(W*SCALE,y*SCALE+.5);ctx.stroke();}}
  ...
  card.querySelector('.count').textContent=state.pixels.size+' märkta pixlar';
  const labels=[...new Set(state.pixels.values())].sort();card.querySelector('.legend').innerHTML=labels.map(l=>'<span class="swatch" style="background:'+colorFor(l)+'">'+l+'</span>').join('');
  refreshGlobal();
 }}
 ...
 canvas.addEventListener('pointerdown',e=>{{state.drag=false;state.last=null;canvas.setPointerCapture(e.pointerId);paint(e);state.drag=true;}});
 canvas.addEventListener('pointermove',e=>{{if(state.drag)paint(e);}});canvas.addEventListener('pointerup',()=>{{state.drag=false;state.last=null;}});
 ...
 card.querySelector('.baseup').onclick=()=>{{b.value=Math.max(0,state.baseline-1);b.onchange();}};card.querySelector('.basedown').onclick=()=>{{b.value=Math.min(H-1,state.baseline+1);b.onchange();}};
 const er=card.querySelector('.erase');er.onclick=()=>{{state.erase=!state.erase;er.textContent='Sudd: '+(state.erase?'på':'av');}};
 card.querySelector('.clear').onclick=()=>{{if(confirm('Rensa alla märkta pixlar i detta ord?')){{state.pixels.clear();render();}}}};

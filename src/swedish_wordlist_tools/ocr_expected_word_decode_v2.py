...
from .ocr_manual_pixel_atlas_match_v4 import BASELINE_ANCHORS, _anchor_model
...
def _beam_decode(
    *,
    expected: str,
    by_pos: dict[int, list[dict[str, object]]],
    beam_width: int,
    ...
    while beams:
        ...
        for b in beams:
            ...
            nxt.append(
                Beam(
                    b.pos + 1,
                    b.score - missing_penalty,
                    b.used,
                    b.last_x,
                    b.picks,
                    b.missing + (expected[b.pos],),
                )
            )
            for h in by_pos.get(b.pos, []):
                ...
                gain = (
                    10.0 * len(pts)
                    + 1.5 * int(h.get("template_source_count", 0))
                    - 2.0 * int(h.get("extra", 0))
                    - float(h.get("score", 0.0))
                )
                nxt.append(
                    Beam(
        ...
        beams = sorted(
            best.values(),
            key=lambda b: (b.pos == len(expected), b.score, len(b.used)),
            reverse=True,
        )[:beam_width]
...
def _split_residual(
    unexplained: set[tuple[int, int]], used: set[tuple[int, int]]
) -> tuple[set[tuple[int, int]], set[tuple[int, int]]]:
    """Split residual ink into attached and orphan pixels.

    Attached residual directly touches a pixel already owned by a decoded glyph
    in the 8-neighbourhood.  This is where serif/raster variation and ink shared
    with a connected neighbour naturally lands.  Orphan residual has no such
    contact and is much stronger evidence that the word is not yet explained.
    """
    ...
    for x, y in unexplained:
        if any(
            ...
            attached.add((x, y))
    return attached, unexplained - attached
    ...
    *,
    ...
    for word in words:
        ...
        if expected == "pl.":
            if max_pl == 0:
                continue
            if max_pl > 0 and counts[expected] >= max_pl:
                continue
        elif max_same_word > 0 and counts[expected] >= max_same_word:
            continue
        counts[expected] += 1
        out.append(word)
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Expected-word decoder v2: attached-vs-orphan residuals and diverse exception batches."
    )
    ap.add_argument("atlas", type=Path)
    ap.add_argument("library", type=Path)
    ap.add_argument("--limit", type=int, default=500, help="Maximum unseen selected target words (0 = all)")
    ap.add_argument("--max-same-word", type=int, default=3)
    ap.add_argument("--max-pl", type=int, default=0, help="Maximum pl. targets; 0 skips pl. entirely")
    ...
    ap.add_argument(
        "--attached-budget-per-pick",
        type=int,
        default=-1,
        help="Allowed attached residual per decoded glyph/cluster; -1 uses --max-extra",
    )
    ...
    attached_budget_per_pick = (
        args.max_extra if args.attached_budget_per_pick < 0 else args.attached_budget_per_pick
    )
    ...
    for word in words:
        ...
        best = max(
            trials,
            key=lambda t: (tuple(t["score_key"]), 1 if t["style"] == manifest_style else 0),
        )
            ...
            results.append(
                {
                    ...
                    "baseline_method": "expected-word-global-v2",
                    ...
                    "decode_trials": {
                        str(t["style"]): {
                            ...
                            "unexplained_pixels": t["unexplained_pixels"],

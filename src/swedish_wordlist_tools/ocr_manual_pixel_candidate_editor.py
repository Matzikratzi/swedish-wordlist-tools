...
document.querySelectorAll('.card').forEach(card=>{{const cv=card.querySelector('canvas'),ctx=cv.getContext('2d'),img=card.querySelector('.src'),W=+card.dataset.w,H=+card.dataset.h;let proposals=JSON.parse(card.dataset.proposals).map((p,i)=>({{...p,id:i,pixels:new Set((p.pixels||[]).map(q=>q[0]+','+q[1]))}}));let state={{selected:proposals.length?0:null,baseline:+card.dataset.baseline}};cards.push([card,proposals,state]);
function draw(){{ctx.imageSmoothingEnabled=false;ctx.clearRect(0,0,cv.width,cv.height);ctx.drawImage(img,0,0,cv.width,cv.height);for(const p of proposals){{ctx.fillStyle=colorFor(p.label)+(p.status==='accepted'?'99':'55');for(const k of p.pixels){{let [x,y]=k.split(',').map(Number);ctx.fillRect((x+MARGIN)*SCALE,(y+MARGIN)*SCALE,SCALE,SCALE)}}}}ctx.strokeStyle='#e00000';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,(state.baseline+MARGIN+1)*SCALE);ctx.lineTo(cv.width,(state.baseline+MARGIN+1)*SCALE);ctx.stroke();renderList()}}

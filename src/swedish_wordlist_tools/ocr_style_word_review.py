...
def main() -> int:
    ...
    for row in words:
        ...
        if not isinstance(raw_segments, list):
            raw_segments = [dict(g, kind="glyph", expected_text=g.get("character", "")) for g in row.get("glyphs", []) if isinstance(g, dict)]

        glyph_html: list[str] = []
        for g in raw_segments:
            if not isinstance(g, dict):
                continue
            kind = str(g.get("kind") or "glyph")
            ch = str(g.get("expected_text") or g.get("character") or "")
            idx = g.get("index", "")
            rel = g.get("file")
            ...
            if kind == "cluster":
                cluster_total += 1
                glyph_html.append(
                    f'<figure class="glyph cluster" data-index="{html.escape(str(idx), quote=True)}" data-char="{html.escape(ch, quote=True)}" data-file="{html.escape(rel, quote=True)}">'
                    f'<div class="gframe"><img src="{_data_uri(path)}" alt="{html.escape(ch)}"></div>'
                    f'<figcaption>{html.escape(ch)}</figcaption>'
                    '<div class="clusterbadge">CLUSTER</div><div class="excluded">ej träningsdata</div>'
                    '</figure>'
                )
            else:
                glyph_html.append(
        ...
        cluster_note = ""
        if int(row.get("cluster_count") or 0):
            cluster_note = f'<span class="cluster-summary">{int(row.get("cluster_count") or 0)} sammanvuxet kluster</span>'
        cards.append(
            ...
            '<header>'
            f'<strong>{html.escape(expected)}</strong><span class="badge">{html.escape(style)}</span><span>subnr {html.escape(str(subnr))}</span>{cluster_note}{source_html}'
            '</header>'
...
function refresh(){{let issues=0;document.querySelectorAll('.word').forEach(w=>{{const issue=isIssue(w);w.classList.toggle('issue',issue);w.querySelectorAll('.glyph:not(.cluster)').forEach(g=>g.classList.toggle('issue',g.querySelector('.glyph-status').value!=='ok'));const textok=w.dataset.search.includes(q.value.toLowerCase());w.classList.toggle('hidden',!textok||(onlyIssues&&!issue));if(issue)issues++;}});summary.textContent=issues+' ord med markerad avvikelse';}}
...
'''
    ...
    print(args.out)
    print(f"words={len(cards)} clusters={cluster_total} missing={missing} styles={','.join(sorted(set(str(w.get('style')) for w in words)))} manifest={manifest_path.name}")
    return 0

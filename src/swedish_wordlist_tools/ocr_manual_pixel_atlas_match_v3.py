...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Baseline-aware manual pixel matcher that also emits topology-rejected candidates for correction."
    )
    ...
    ap.add_argument("--rejected-per-label", type=int, default=4)
    ...
    accepted_words: Counter[tuple[str, str]] = Counter()
    rejected_words: Counter[tuple[str, str]] = Counter()
    ...
    for word in words:
        ...
        for label, label_models in style_models.items():
            ok_candidates: list[dict[str, object]] = []
            bad_candidates: list[dict[str, object]] = []
            for model_index, model in enumerate(label_models):
                for raw in _scan_model(
                    ...
                    if int(hit["external_contacts"]) <= args.max_external_contacts:
                        hit["candidate_status"] = "accepted"
                        ok_candidates.append(hit)
                    else:
                        hit["candidate_status"] = "topology-rejected"
                        bad_candidates.append(hit)
            ...
            ok_candidates.sort(key=sort_key)
            bad_candidates.sort(key=sort_key)
            chosen_ok = _nonoverlap(ok_candidates, args.hits_per_label)
            chosen_bad = _nonoverlap(bad_candidates, args.rejected_per_label)
            if chosen_ok:
                accepted[label] = chosen_ok
                accepted_words[(style, label)] += 1
            if chosen_bad:
                rejected[label] = chosen_bad
                rejected_words[(style, label)] += 1
    ...
    payload = {
        "format": "saol-manual-pixel-transfer-v3",
        ...
        "accepted_words_by_label": nested(accepted_words),
        "topology_rejected_words_by_label": nested(rejected_words),
        "settings": {

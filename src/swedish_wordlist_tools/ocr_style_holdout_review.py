...
def main() -> int:
    ...
    for r in bench.get("results", []):
        ...
        searchable = f"{style} {expected} {r.get('page')} {r.get('subnr')} {sid}".lower()
        cards.append(
            f'<article class="word {"okword" if r.get("correct") else "badword"}" data-source-id="{sid}" data-search="{_h(searchable)}">'
            f'<header><strong>{_h(expected)}</strong><span>→</span><strong>{_h(prediction)}</strong><span class="badge">{_h(style)}</span><span>source {sid}</span><span>sida {_h(r.get("page"))}</span></header>'
